...
def _build_interaction_explanations(
    ...
    SHAP interaction values require a purely numeric DataFrame.
    Categorical columns are label-encoded to integers before the call
    so XGBoost's DMatrix does not raise a dtype error.
    ...
    try:
        # ------------------------------------------------------------------
        # Convert categoricals to their integer codes so SHAP can build
        # the DMatrix without needing enable_categorical=True.
        # This is safe because TreeExplainer only needs the numeric split
        # points — the category strings are already encoded inside the
        # trained booster.
        # ------------------------------------------------------------------
        X_numeric = X_inference.copy()
        for col in X_numeric.columns:
            if hasattr(X_numeric[col], 'cat'):
