...
(() => {
  ...
  // ------------------------------------------------------------------ //
  // Reset result UI — hides all result panels and clears charts.
  // Called whenever a submission fails so stale data from a prior
  // successful call is never left on screen.
  // ------------------------------------------------------------------ //
  function resetResultUI() {
    // Hide content panels, show placeholders
    ...
    // Purge every Plotly chart so the canvases are blank (not just hidden)
    const chartIds = [
      'gauge-chart',
      'denial-reason-chart',
    ...
    // Reset KPI cards
    ['kpi-prob', 'kpi-shap', 'kpi-flags', 'kpi-revised'].forEach(id => {
      _setKPI(id, '—', 'neutral', '—');
    });
  }
  ...
  function renderOverview(payload, result) {
    ...
    if (ab) {
      ...
      ab.innerHTML = `<strong>${result.risk_level} likelihood (${result.denial_probability_pct.toFixed(1)}%)</strong>${result.action_recommendation}`;
    }

    // Denial likelihood factor distribution — pass ALL features so the chart
    // can filter to positive-SHAP ones and show a complete distribution
    const allFeatures = result?.explanation?.all_features ?? null;
    Charts.denialReasonBar('denial-reason-chart', allFeatures);
  }

  // ------------------------------------------------------------------ //
  // SHAP Analysis tab render — uses REAL API SHAP values
  // ------------------------------------------------------------------ //
  function renderSHAPTab(payload, result) {
    // Get real SHAP contributions from the API response
    ...
    const HIGH_THRESHOLD = 0.05;  // lower threshold — real SHAP values are smaller
    ...
    _setKPI('kpi-prob',
      `${prob.toFixed(1)}%`,
      riskClass,
      prob > 42 ? '↑ Above 42% average' : '↓ Below 42% average'
    );

    _setKPI('kpi-shap',
      `${netShap >= 0 ? '+' : ''}${netShap.toFixed(3)}`,
      netShap > 0 ? 'high' : 'green',
      `Model base: ${baseVal.toFixed(3)}`
    );
    ...
    // Revised risk: remove top driver's contribution from log-odds
    ...
    // Find real SHAP value for claim_net_log feature to anchor the dependence chart
    const claimNetFeature = contributions.find(c =>
      c.name.toLowerCase().includes('net') || c.name.toLowerCase().includes('claim')
    );
    const claimNetShap = claimNetFeature?.value ?? null;
