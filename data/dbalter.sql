...
-----------------------------------------------denialprediction_final.sql------------------------------------------------

UPDATE denial_prediction_final
SET cpt_category = CASE
    WHEN activity_code ~ '^[0-9]+$' THEN
        CASE
            WHEN CAST(activity_code AS INTEGER) BETWEEN 100 AND 1999
                THEN 'Anesthesia'
            WHEN CAST(activity_code AS INTEGER) BETWEEN 10000 AND 69999
                THEN 'Surgery'
            WHEN CAST(activity_code AS INTEGER) BETWEEN 70000 AND 79999
                THEN 'Radiology'
            WHEN CAST(activity_code AS INTEGER) BETWEEN 80000 AND 89999
                THEN 'Pathology_Laboratory'
            WHEN CAST(activity_code AS INTEGER) BETWEEN 99202 AND 99499
                THEN 'Evaluation_Management'
            WHEN CAST(activity_code AS INTEGER) >= 90000
                THEN 'Medicine'
            ELSE 'Unknown_CPT'
        END
    ELSE 'HCPCS_Supplies'
